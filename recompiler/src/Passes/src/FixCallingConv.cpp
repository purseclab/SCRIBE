...
//===----------------------------------------------------------------------===//
namespace {
  struct FixCallingConv : public FunctionPass {
    static char ID; // Pass identification, replacement for typeid
    FixCallingConv() : FunctionPass(ID) {}
      ...
      for (auto BB = F.begin(); BB != F.end(); ++BB) {
        for (auto I = BB->begin(); I != BB->end(); ++I) {
          if (auto *Call = dyn_cast<CallInst>(I)) {
            ...
            if (!Callee)
              continue;
            if (Callee->getName() == "print_struct") {
...
char FixCallingConv::ID = 0;
static RegisterPass<FixCallingConv> X("fix-calling-conv", "TODO");
