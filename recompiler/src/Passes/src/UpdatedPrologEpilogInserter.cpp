...
// RECOMP
#define DEBUG_TYPE "updated-prologepilog"
...
private:
  ...
  // RECOMP
  void recompInsertStackLayout(MachineFunction &MF);
...
char PEI::ID = 0;
// RECOMP
static RegisterPass<PEI> X("updated-prologepilog", "updated prologepilog pass");
...
bool PEI::runOnMachineFunction(MachineFunction &MF) {
  ...
  // RECOMP: Read the stack layout from the file, and pre-assign the stack offsets
  recompInsertStackLayout(MF);

  // Calculate actual frame offsets for all abstract stack objects...
  calculateFrameObjectOffsets(MF);

  // RECOMP
  // When there exist callee-saved registers, the function prologue will be
  //   push rbp
  //   mov rbp, rsp
  ...
  //   pop rbp
  // with our fix, we modified push to mov, thus not changing rsp,
  // so we need to adjust the stack size here
  MF.getFrameInfo().setStackSize(MF.getFrameInfo().getStackSize() +
                                  MF.getFrameInfo().getCVBytesOfCalleeSavedRegisters());
...
void PEI::spillCalleeSavedRegs(MachineFunction &MF) {
  ...
  if (!F.hasFnAttribute(Attribute::Naked)) {
    ...
    // RECOMP: make sure it won't conflict with the input stack layout
    // for simplicity, just make them unfixed
    for (auto &CS : CSI) {
      if (CS.isSpilledToReg()) {
        continue;
      }
      int FI = CS.getFrameIdx();
      errs() << "Creating stack object for FI(" << FI << ") with size "
             << MFI.getObjectSize(FI) << " and align "
             << MFI.getObjectAlign(FI).value() << "\n";
      auto newFI = MFI.CreateStackObject(MFI.getObjectSize(FI),
                                         MFI.getObjectAlign(FI), true);
      CS.setFrameIdx(newFI);
      // Ideally we should remove the old stack object here,
      // but that may cause other issues because some code
      // assume there will be no dead object at current stage, 
      // not removing it won't cause any issue afaik
    }
...
void replaceFrameIdx(MachineFunction &MF, int from, int to) {
  const TargetInstrInfo &TII = *MF.getSubtarget().getInstrInfo();
  for(auto &BB : MF) {
    for(auto I = BB.begin(); I != BB.end(); ++I) {
      ...
      if (TII.isFrameInstr(*I))
        continue;
      for (unsigned i = 0, e = MI.getNumOperands(); i != e; ++i)
        if (MI.getOperand(i).isFI() && (MI.getOperand(i).getIndex() == from)) {
          MI.getOperand(i).setIndex(to);
        }
    }
  }
}

void fixStackObject(MachineFunction &MF, int FrameIdx, int Offset, int Size) {
  MachineFrameInfo &MFI = MF.getFrameInfo();
  const TargetFrameLowering *TFI = MF.getSubtarget().getFrameLowering();
  ...
  if (!Exists) {
    int FixedFrameIdx = MFI.CreateFixedObject(Size, Offset, false);
    MFI.setObjectAlignment(FixedFrameIdx, Align(1));
    replaceFrameIdx(MF, FrameIdx, FixedFrameIdx);

    // the final offset being applied is TFI->getFrameIndexReference(...) instead of Offset
    // I'm not sure how exactly getFrameIndexReference works so I will just offset it
    Register BasePtr;
    MFI.setObjectOffset(FixedFrameIdx, Offset + Offset - TFI->getFrameIndexReference(MF, FixedFrameIdx, BasePtr).getFixed());
    
    // No idea why RemoveStackObject segfault, so I will just set the size to 0 which also works
    // MFI.RemoveStackObject(FrameIdx);
    MFI.setObjectSize(FrameIdx, 0);
  }
}

void PEI::recompInsertStackLayout(MachineFunction &MF) {
  MachineFrameInfo &MFI = MF.getFrameInfo();
  auto StkLocMap = parseStkLoc(MF.getName());
  auto is_original_func_bp_based = isBpBased(MF.getName());
  auto is_current_func_bp_based = MF.getFunction().getFnAttribute("frame-pointer").getValueAsString().str() == "all";

  // change current function to bp based
  if (!is_current_func_bp_based) {
    MF.getFunction().addFnAttr("frame-pointer", "all");
    MF.getFunction().addFnAttr("no-frame-pointer-elim", "true");
  }

  if (!is_original_func_bp_based) {
    int max = std::numeric_limits<int>::min();
    // for sp based frame, stk offset will be positive, we need to negate it
    for (int I = 0, E = MFI.getObjectIndexEnd(), ID = 0; I < E; ++I, ++ID) {
      ...
      for (const auto &VI : MF.getVariableDbgInfo()) {
        if (VI.Var && VI.Slot == ID && StkLocMap.count(VI.Var->getName())) {
          if (max < MFI.getObjectSize(I) + StkLocMap[VI.Var->getName()]) {
            max = MFI.getObjectSize(I) + StkLocMap[VI.Var->getName()];
          }
        }
      }
    }
  ...
  for (int I = 0, E = MFI.getObjectIndexEnd(), ID = 0; I < E; ++I, ++ID) {
    ...
    for (const auto &VI : MF.getVariableDbgInfo()) {
      if (VI.Var && VI.Slot == ID && StkLocMap.count(VI.Var->getName())) {
        errs() << "Found " << VI.Var->getName() << " at " << StkLocMap[VI.Var->getName()] << "\n";
        fixStackObject(MF, I, StkLocMap[VI.Var->getName()], MFI.getObjectSize(I));
      }
    }
  }
}
