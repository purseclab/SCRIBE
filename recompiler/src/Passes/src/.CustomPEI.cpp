...
#include <fstream>
#include <string>
#include "llvm/CodeGen/PrologueEpilogInserter.h"
...
//===----------------------------------------------------------------------===//
class CustomPEI : public PrologueEpilogInserter {
public:
    // Required for LLVM's pass mechanism
    static char ID;
    CustomPEI() : PrologueEpilogInserter(ID) {}
    ...
    // Override PEI::replaceFrameIndices()
    void replaceFrameIndices(MachineFunction &MF) override {
...
char CustomPEI::ID = 0;
static RegisterPass<CustomPEI> X("updated-prologepilog", "updated prologepilog pass");
