...
private:
  ...
  uint64_t precalculateStackSize(MachineFunction &MF);
};
...
bool PEI::runOnMachineFunction(MachineFunction &MF) {
  ...
  // Han: Precalculate Stacksize
  uint64_t StackSize_ = precalculateStackSize(MF);
  MF.getFrameInfo().setStackSize(StackSize_);
  MF.getFrameInfo().setOffsetAdjustment(-StackSize_);
...
static void assignCalleeSavedSpillSlots(MachineFunction &F,
  ...
  if (!TFI->assignCalleeSavedSpillSlots(F, RegInfo, CSI, MinCSFrameIndex,
                                        MaxCSFrameIndex)) {
    // https://github.com/llvm/llvm-project/blob/main/llvm/lib/Target/X86/X86FrameLowering.cpp#L2686 MODIFY IT HERE
...
void PEI::spillCalleeSavedRegs(MachineFunction &MF) {
  ...
  if (!F.hasFnAttribute(Attribute::Naked)) {
    ...
    if (!CSI.empty()) {
      ...
      for (MachineBasicBlock *SaveBlock : SaveBlocks)
        insertCSRSaves(*SaveBlock, CSI); // TODO: here

      // Update the live-in information of all the blocks up to the save point.
      updateLiveness(MF);

      for (MachineBasicBlock *RestoreBlock : RestoreBlocks)
        insertCSRRestores(*RestoreBlock, CSI); // TODO: here
    }
  }
}
...
//===----------------------------------------------------------------------===//
  ...
  uint64_t PEI::precalculateStackSize(MachineFunction &MF) {
    const TargetInstrInfo &TII = *MF.getSubtarget().getInstrInfo();
    const TargetRegisterInfo &TRI = *MF.getSubtarget().getRegisterInfo();
    // const TargetFrameLowering *TFI = MF.getSubtarget().getFrameLowering();
    MachineFrameInfo &MFI = MF.getFrameInfo();
    ...
    int calleeSavedSize = MFI.getCalleeSavedInfo().size() * MF.getDataLayout().getPointerSize();
    // ^ spilled registers took calleeSavedSize bytes
    if (smallest > -calleeSavedSize) {
      smallest = -calleeSavedSize;
    }
      ...
      for(auto I = BB.begin(); I != BB.end(); ++I) {
        ...
        for (unsigned i = 0, e = MI.getNumOperands(); i != e; ++i) {
          ...
          if (!(FrameIdxOffsetMap.count(FrameIdx))) {
            ...
            if (Offset <= 0 && StackSize < -(Offset - Size)) {
  ...
  void PEI::replaceFrameIndices(MachineFunction &MF) {
    ...
    errs() << "CalleeSavedSize: " << calleeSavedSize << "\n";
